    ...
    success: function (result) {
      ...
      if (resultCode == 200) {
        /*$.each(result.data, function (index) {
          ...
          <button type="button" onclick="departmentCheck(${result.data[index].id})" class="btn btn-primary btn-sm deleteDepartmentBtn"  data-id="${result.data[index].id}">
        ...
        //result.data.forEach(function (result) {
          ...
          var deleteButton = $("<button>").addClass("btn btn-primary btn-sm me-2")
            .attr("onclick", "checkDepartment(id)")
            .attr("type", "button")
            .attr("data-id", id)
            .html('<i class="fa-solid fa-trash fa-fw"></i>');
...
function checkDepartment(e) {
  ...
  $.ajax({
    ...
    dataType: "json",
    data: {
      id: e // Retrieves the data-id attribute from the calling button
    },
    success: function (result) {
      if (result.status.code == 200) {
        if (result.data.department[0].personnelCount == 0) {
          $("#areYouSureDeptName").text(result.data.department[0].departmentName);

          $("#areYouSureDeleteDepartmentModal").modal("show");
        } else {
          $("#cantDeleteDeptName").text(result.data.department[0].departmentName);
          $("#personnelCount").text(result.data.department[0].personnelCount);
